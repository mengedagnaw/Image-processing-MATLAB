...
%
% Notes:
% - This script is self-contained (includes a simple imoverlay helper).
% - If you want GitHub-friendly relative paths, see the "PATH OPTIONS" section below.
...
%  =========================
% Option A (your current MATLAB Drive absolute path):
imgPath = "/MATLAB Drive/crack3.png";

% Option B (GitHub-friendly relative path):
% Put the image in: images/crack3.png (relative to this .m file folder)
...
I_gray = im2gray(I);
figure('Name','Grayscale'); imshow(I_gray);
...
overlay = imoverlay_simple(I_gray, logI, [1 0 0]); % red overlay
figure('Name','LoG edges overlay'); imshow(overlay);
title('LoG Edges Overlay (Red)');

%% =========================
% Helper: imoverlay_simple
% =========================
function out = imoverlay_simple(imgGray, mask, colorRGB)
%IMOVERLAY_SIMPLE Create an RGB overlay from a grayscale image + binary mask.
%   out = imoverlay_simple(imgGray, mask, colorRGB)
%   - imgGray: grayscale image (uint8/uint16/double/etc.)
%   - mask: logical/binary edge map
%   - colorRGB: 1x3 RGB values in [0,1], e.g. [1 0 0] for red
